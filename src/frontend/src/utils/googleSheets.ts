const SERVICE_ACCOUNT_EMAIL =
  "face-attendance@face-attendance-496305.iam.gserviceaccount.com";

// PRIVATE_KEY uses a template literal so each PEM line is a real newline in the source.
// The .replace(/\\n/g,'\n') fallback handles any escaped \n that might sneak in.
...
export async function createSignedJwt(): Promise<string> {
  ...
  const signature = await crypto.subtle.sign(
    ...
    privateKey,
    enc.encode(sigInput).buffer as ArrayBuffer,
  );
